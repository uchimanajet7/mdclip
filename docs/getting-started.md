...
取得した source archive は release tag に対応する固定のソースです。新しい version を使う場合は、最新 GitHub Release の `Source code (zip)` を取得し直し、必要に応じて `npm ci` を実行してから、再度 `npm run dev` を実行します。
...
この手順では、現在の MdClip 依存関係に合わせて Node.js `22.22.2` 以上、npm `7` 以上が使える状態を前提にします。Node.js が未導入、または version が古い場合は [Node.js 公式 download page](https://nodejs.org/en/download) から新しい Node.js を install します。
