...
4. Raycast publish 用 GitHub token の認証ユーザーを Git identity として設定する。
5. `npm run publish`
...
`Publish Release to Raycast` workflow は、`npm run publish` の前に `RAYCAST_PUBLISH_GITHUB_TOKEN_CLASSIC` を GitHub CLI の `GH_TOKEN` として使用し、認証ユーザーの `login` と `id` を取得する。その値から Git の `user.name` と `user.email` を設定する。
...
Raycast CLI は publish 中に `raycast/extensions` の fork を clone し、その clone 上で commit と push を行う。そのため、GitHub Actions runner 上でも `npm run publish` の前に Git identity を設定する。
...
`GH_TOKEN` は GitHub CLI 用の認証変数である。`Publish Release to Raycast` workflow では、GitHub Release の存在確認と、Raycast publish 用 GitHub token の認証ユーザー取得に使用する。Raycast CLI の publish 認証には `GITHUB_ACCESS_TOKEN` を使用する。
...
`npm run publish` の前に Git identity が設定されていない場合、Raycast CLI が publish 中に `git config --get user.name` または `git config --get user.email` を実行した時点で失敗する。workflow では `RAYCAST_PUBLISH_GITHUB_TOKEN_CLASSIC` の認証ユーザーを使って Git identity を設定する。
...
- `Publish Release to Raycast` workflow が、`npm run publish` の前に Git identity を設定する。
