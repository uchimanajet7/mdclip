...
| Distribution model         | public GitHub repository plus local Raycast development mode |
| Active release unit        | GitHub Releases                                              |
| Raycast Store publish      | Inactive unless explicitly re-approved                       |
...
| Markdown Source N Name   | textfield |       no | none    | Raycast 上の表示名。空の場合は folder name を使う           |
| Editor                   | appPicker |       no | none    | Open in Editor で使う editor                                |
| Preview Line Count       | textfield |       no | `10`    | preview に表示する冒頭行数                                  |
| Preview Max Characters   | textfield |       no | `4000`  | preview の最大文字数                                        |
...
preview は `Preview Line Count` と `Preview Max Characters` の小さい方の制限に従って切り詰めます。
...
| Command                       | 役割                                      |
| ----------------------------- | ----------------------------------------- |
| `npm run lint`                | 通常の local verification                 |
...
manual UI verification では、command names、preferences、list behavior、copy actions、preview、editor actions、error states が MdClip / Markdown Source model と一致することを確認します。

## 18. Release and Store path

Active public release unit は GitHub Release です。

Raycast Store publish は inactive です。Store publish を再開する場合は、product direction、workflow、script、secret、Store Version History、screenshots、README、docs、GitHub About metadata をまとめて再承認します。

root `CHANGELOG.md` は Raycast Store Version History 用の active-looking surface にならないように root から外します。Store publish 用の README、Version History、screenshot 手順、背景説明は `raycast-publish/` にまとめます。現在の GitHub Release 作成では `.github/release-changelog/*.md` を release body として使います。
...
Current screenshot and README media handling is defined in `docs/screenshot-media.md`.
...
Old Store-oriented screenshots and media remain historical only until current MdClip evidence exists and cleanup is separately approved.
