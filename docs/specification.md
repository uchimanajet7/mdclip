...
`docs/run-from-source.md` は、Raycast Store を使わずに `Local Copy Blocks` をソースコードから自分の Mac に読み込む利用者向け手順とする。

この手順では、Raycast、Node.js、npm、必要に応じた Git の準備、ソースコード取得、`npm ci`、`npm run dev`、Block Set 設定、動作確認、ソースコード更新、Extension とソースコードと追加ツールの削除を扱う。

`docs/run-from-source.md` は、まっさらな環境で使い始める利用者が、利用を継続しない場合に導入前の状態へ戻せることを重視する。
